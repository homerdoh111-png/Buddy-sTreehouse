...
function App() {
  ...
  const getSpeechText = () => {
    const hunger = buddy.needs?.hunger ?? 80;
    const energy = buddy.needs?.energy ?? 80;
    const happiness = buddy.needs?.happiness ?? 80;
    
    if (hunger < 30) return "I'm hungry! 🍎";
    if (energy < 30) return "I'm so tired... 😴";
    if (happiness < 50) return "Let's play together! 🎮";
  ...
  const happiness = buddy.needs?.happiness ?? 85;
  const hunger = buddy.needs?.hunger ?? 80;
  const energy = buddy.needs?.energy ?? 90;
  const level = buddy.level ?? 1;
    ...
    <div className="relative w-full h-screen overflow-hidden bg-gradient-to-b from-sky-200 to-green-100">
      ...
      >
        <div className="bg-gradient-to-br from-purple-400/30 via-pink-400/30 to-purple-500/30 backdrop-blur-sm rounded-3xl p-6 shadow-2xl border-2 border-white/20 w-80">
          ...
          <div className="space-y-4 mb-6">
            <div>
              <div className="flex justify-between mb-2">
                ...
                <span className="text-white/90 text-sm font-bold">{Math.round(happiness)}%</span>
              </div>
            ...
            <div>
              <div className="flex justify-between mb-2">
                ...
                <span className="text-white/90 text-sm font-bold">{Math.round(hunger)}%</span>
              </div>
              <div className="h-2 bg-white/20 rounded-full overflow-hidden">
                <div 
            ...
            <div>
              <div className="flex justify-between mb-2">
                ...
                <span className="text-white/90 text-sm font-bold">{Math.round(energy)}%</span>
              </div>
              <div className="h-2 bg-white/20 rounded-full overflow-hidden">
                <div 
          ...
          {/* Quick Actions - ONLY HERE, not duplicated! */}
          <div className="grid grid-cols-3 gap-3">
            <button
              onClick={() => buddy.feedBuddy?.('apple')}
      ...
      {/* NO Bottom Quick Actions - removed duplicate! */}
