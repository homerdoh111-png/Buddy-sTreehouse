...
// Activity Button - NO BUBBLES, JUST BUTTON
  ...
  return (
    <Float speed={1.5} rotationIntensity={0.1} floatIntensity={0.6}>
      <group position={position}>
        {/* HUGE HTML button - NO bubble sphere! */}
        <Html
          center
          distanceFactor={2.5} // Was 4, now 2.5 (MUCH bigger!)
          style={{
        ...
        >
          <div
            ...
            className={`
              w-72 h-72 rounded-3xl flex flex-col items-center justify-center gap-6
              transition-all shadow-2xl border-4
              ${activity.unlocked 
                ? 'bg-white/95 hover:bg-white cursor-pointer hover:scale-110 border-yellow-400/80 hover:shadow-[0_0_50px_rgba(250,204,21,0.6)]' 
                : 'bg-gray-400/70 cursor-not-allowed border-gray-500/50'
              }
            `}
          >
            <span className="text-8xl drop-shadow-lg">{activity.icon}</span>
...
function TreehouseModel() {
  ...
  return (
    <primitive 
      object={scene} 
      scale={26}  // Was 15, now 26 (70% bigger!)
      position={[0, -5, -14]}
      rotation={[0, 0, 0]}
    />
  );
...
function BuddyModel({ onClick }: { onClick?: () => void }) {
  ...
  return (
    <Float speed={2} rotationIntensity={0.2} floatIntensity={0.5}>
      <group ref={groupRef} position={[-5, 0, -5]} onClick={handleClick}>
        <primitive object={scene} scale={10} />  {/* Was 8, now 10 (30% bigger!) */}
      </group>
    </Float>
  );
}

// Star Particles ONLY (no big bubbles)
...
function SimpleLighting() {
  return (
    <>
      ...
      <pointLight position={[0, 4, -14]} intensity={3} color="#FFD700" distance={20} />
    </>
  );
}
...
export default function Buddy3D({ onInteraction, interactive = true, onActivityClick }: Buddy3DProps) {
  ...
  // Activities positioned around treehouse - SPREAD OUT MORE
  const activities = [
    { 
      id: 'letters', 
      name: 'Letters', 
      icon: '📚', 
      position: [-10, 3, -14] as [number, number, number],
      unlocked: true 
    },
    { 
      id: 'numbers', 
    ...
    { 
      ...
      name: 'Shapes', 
      icon: '⭐', 
      position: [-8, -1, -13] as [number, number, number],
      unlocked: false 
    },
    { 
      id: 'music', 
