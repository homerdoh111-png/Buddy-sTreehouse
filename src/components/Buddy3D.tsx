...
// Responsive Forest Environment - Adapts to screen size
function ForestEnvironment() {
  ...
  const responsiveScale = Math.max(12, viewport.width * 0.8);
  
  return (
    <group ref={groupRef}>
      <primitive 
        object={scene} 
        scale={responsiveScale}  // DYNAMIC - scales with screen!
        position={[0, -3, 0]}
        rotation={[0, 0, 0]}
      />
    </group>
      ...
      {[...Array(20)].map((_, i) => (
        <mesh key={i}>
          ...
          <meshStandardMaterial 
            ...
            toneMapped={false}
          />
          <pointLight color="#FFE8A1" intensity={0.5} distance={2} />
        </mesh>
      ))}
    </group>
  );
      ...
      {[...Array(12)].map((_, i) => {
        ...
        return (
          ...
          >
            ...
            <meshBasicMaterial color="#FFE8CC" transparent opacity={0.08} side={THREE.DoubleSide} blending={THREE.AdditiveBlending} />
          </mesh>
        );
      })}
    </group>
...
function DreamlikeLighting() {
  return (
    <>
      <directionalLight position={[-8, 15, -3]} intensity={1.8} castShadow shadow-mapSize={[4096, 4096]} shadow-camera-left={-20} shadow-camera-right={20} shadow-camera-top={20} shadow-camera-bottom={-20} color="#FFE8CC" />
      <directionalLight position={[10, 8, -5]} intensity={0.8} color="#FFD4A3" />
      <directionalLight position={[0, 10, 10]} intensity={0.6} color="#C8E6FF" />
      <ambientLight intensity={0.7} color="#FFE8CC" />
      <hemisphereLight args={['#B8E6FF', '#8B6F47', 0.8]} />
...
export default function Buddy3D({ onInteraction, interactive = true }: Buddy3DProps) {
  ...
  return (
    <div className="w-full h-full absolute inset-0">
      ...
      >
        <PerspectiveCamera makeDefault position={[0, 2, 10]} fov={65} />
        <ResponsiveCamera />
        ...
        <EffectComposer>
          ...
          <Bloom intensity={1.2} luminanceThreshold={0.6} luminanceSmoothing={0.9} mipmapBlur />
          <DepthOfField focusDistance={0.015} focalLength={0.05} bokehScale={3} />
          <Vignette offset={0.15} darkness={0.35} />
        </EffectComposer>
