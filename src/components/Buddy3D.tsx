...
// MASSIVE Activity Button - NO 3D ELEMENTS AT ALL
function ActivityBubble({ 
  ...
  // NO Float, NO 3D meshes - just HTML in space!
  return (
    <group position={position}>
      {/* ONLY HTML - NO 3D ELEMENTS! */}
      <Html
        center
        distanceFactor={0.8} // Even closer! (was 1.2, now 0.8)
        style={{
      ...
      >
        <div
          ...
          className={`
            w-[500px] h-[500px] rounded-3xl flex flex-col items-center justify-center gap-10
            transition-all shadow-2xl border-8
            ${activity.unlocked 
              ? 'bg-white/98 hover:bg-white cursor-pointer hover:scale-105 border-yellow-400' 
              : 'bg-gray-400/85 cursor-not-allowed border-gray-500'
            }
          `}
        >
          <span className="text-[150px] drop-shadow-lg">{activity.icon}</span>
          <span className="text-5xl font-bold text-gray-800">{activity.name}</span>
        </div>
      </Html>
    </group>
  );
...
// ONLY Star Particles - nothing else!
...
export default function Buddy3D({ onInteraction, interactive = true, onActivityClick }: Buddy3DProps) {
  ...
  // Activities positioned around treehouse
  const activities = [
    { 
      id: 'letters', 
      name: 'Letters', 
      icon: '📚', 
      position: [-14, 4, -16] as [number, number, number],
      unlocked: true 
    },
    { 
      id: 'numbers', 
    ...
    { 
      ...
      name: 'Colors', 
      icon: '🎨', 
      position: [0, 11, -14] as [number, number, number],
      unlocked: true 
    },
    { 
      id: 'shapes', 
      name: 'Shapes', 
      icon: '⭐', 
      position: [-11, -2, -15] as [number, number, number],
      unlocked: false 
    },
    { 
      id: 'music', 
    ...
    <div className="w-full h-full absolute inset-0">
      ...
      >
        ...
        <Suspense fallback={<LoadingScene />}>
          ...
          {/* Activity buttons - NO Float, just positioned */}
