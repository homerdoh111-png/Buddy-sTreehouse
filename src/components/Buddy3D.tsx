...
// Enhanced Forest Environment Model
  ...
  useEffect(() => {
    scene.traverse((child) => {
      if (child instanceof THREE.Mesh) {
        ...
        if (child.material) {
          // Make materials more vibrant and dreamlike
...
function BuddyModel({ onClick }: { onClick?: () => void }) {
  ...
  return (
    <Float speed={2} rotationIntensity={0.2} floatIntensity={0.4}>
      <group ref={groupRef} position={[2.5, 0.5, -3]} onClick={handleClick}>
...
function MagicalParticles() {
  ...
  for (let i = 0; i < particleCount; i++) {
    ...
    if (colorChoice < 0.3) {
      colors[i * 3] = 1; colors[i * 3 + 1] = 0.95; colors[i * 3 + 2] = 0.6; // Gold
    } else if (colorChoice < 0.6) {
      colors[i * 3] = 0.8; colors[i * 3 + 1] = 1; colors[i * 3 + 2] = 0.9; // Mint
    } else {
      colors[i * 3] = 1; colors[i * 3 + 1] = 0.7; colors[i * 3 + 2] = 1; // Pink
    }
...
function DreamlikeLighting() {
  return (
    <>
      ...
      {/* Hemisphere for dreamy atmosphere */}
      <hemisphereLight 
  ...
  return (
    <group>
      ...
      <mesh position={[0, 0, 0]} rotation={[-Math.PI / 2, 0, 0]}>
        ...
        <meshStandardMaterial color="#3d5a3d" wireframe />
      </mesh>
    </group>
  );
}
    ...
    <div className="w-full h-full">
      ...
      >
        ...
        <EffectComposer>
          {/* Color Enhancement */}
          <HueSaturation 
            saturation={0.15}  // 15% more vibrant
            hue={0}
          />
          
          <BrightnessContrast
            brightness={0.05}  // Slightly brighter
            contrast={0.1}     // More contrast
          />
