...
// Sharp 360° Background with High-Res Settings
...
// Floating Activity Bubble Component
        ...
        >
          <div
            ...
            className={`
              ...
              transition-all shadow-2xl border-2
              ${activity.unlocked 
                ? 'bg-white/90 hover:bg-white cursor-pointer hover:scale-110 border-white/50' 
                : 'bg-gray-400/50 cursor-not-allowed border-gray-400/30'
              }
            `}
          >
            <span className="text-4xl">{activity.icon}</span>
            <span className="text-xs font-bold text-gray-700">{activity.name}</span>
          </div>
        </Html>
      </group>
    </Float>
...
function BuddyModel({ onClick }: { onClick?: () => void }) {
  ...
  return (
    <Float speed={2} rotationIntensity={0.2} floatIntensity={0.4}>
      <group ref={groupRef} position={[-2, 0, -2]} onClick={handleClick}>
        <primitive object={scene} scale={3.5} />  {/* Was 2.5, now 3.5 (40% bigger!) */}
      </group>
    </Float>
  );
}
...
function SimpleLighting() {
  return (
    <>
      ...
      <pointLight position={[0, 2, -8]} intensity={2} color="#FFD700" distance={10} />
    </>
  );
}
