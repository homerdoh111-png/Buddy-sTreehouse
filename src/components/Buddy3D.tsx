...
// MASSIVE Activity Button - NO BOXES OR EFFECTS
  ...
  return (
    <Float speed={1} rotationIntensity={0.05} floatIntensity={0.3}>
      <group position={position}>
        {/* MASSIVE HTML button - 200% larger! */}
        <Html
          center
          distanceFactor={1.2} // Was 2.5, now 1.2 (MUCH bigger!)
          style={{
        ...
        >
          <div
            ...
            className={`
              w-96 h-96 rounded-3xl flex flex-col items-center justify-center gap-8
              transition-all shadow-2xl border-4
              ${activity.unlocked 
                ? 'bg-white/98 hover:bg-white cursor-pointer hover:scale-105 border-yellow-400/90' 
                : 'bg-gray-400/80 cursor-not-allowed border-gray-500/60'
              }
            `}
          >
            <span className="text-9xl drop-shadow-lg">{activity.icon}</span>
            <span className="text-3xl font-bold text-gray-800">{activity.name}</span>
          </div>
        </Html>
      </group>
    </Float>
...
// Treehouse - 30% BIGGER than before!
function TreehouseModel() {
  ...
  return (
    <primitive 
      object={scene} 
      scale={34}  // Was 26, now 34 (30% bigger!)
      position={[0, -6, -16]}
      rotation={[0, 0, 0]}
    />
  );
  ...
  return (
    <Float speed={2} rotationIntensity={0.2} floatIntensity={0.6}>
      <group ref={groupRef} position={[-6, 0, -6]} onClick={handleClick}>
        <primitive object={scene} scale={15} />  {/* Was 10, now 15 (50% bigger!) */}
      </group>
    </Float>
  );
}

// ONLY Star Particles - NO boxes, NO other effects!
...
export default function Buddy3D({ onInteraction, interactive = true, onActivityClick }: Buddy3DProps) {
  ...
  // Activities positioned around treehouse - WELL SPREAD
  const activities = [
    { 
      id: 'letters', 
      name: 'Letters', 
      icon: '📚', 
      position: [-12, 4, -16] as [number, number, number],
      unlocked: true 
    },
    { 
      id: 'numbers', 
    ...
    { 
      ...
      name: 'Colors', 
      icon: '🎨', 
      position: [0, 10, -14] as [number, number, number],
      unlocked: true 
    },
    { 
      id: 'shapes', 
      name: 'Shapes', 
      icon: '⭐', 
      position: [-10, -2, -15] as [number, number, number],
      unlocked: false 
    },
    { 
      id: 'music', 
